...
@Slf4j
public class Socks4CommandRequestHandler extends SimpleChannelInboundHandler<DefaultSocks4CommandRequest> {
    static EventLoopGroup group = new NioEventLoopGroup();

    @Override
    protected void channelRead0(ChannelHandlerContext ctx, DefaultSocks4CommandRequest msg) throws Exception {
        ...
        if(msg.type().equals(Socks4CommandType.CONNECT)) {
            log.trace("准备连接目标服务器");
            ...
            bootstrap.connect(address.getHostAddress(), msg.dstPort())
                    .addListener((ChannelFutureListener) future -> {
                        if(future.isSuccess()) {
                            ...
                            Socks4CommandResponse commandResponse =
                                    new DefaultSocks4CommandResponse(Socks4CommandStatus.SUCCESS, address.getHostAddress(),msg.dstPort());
                            ctx.writeAndFlush(commandResponse);
                        } else {
                            Socks4CommandResponse commandResponse =
                                    new DefaultSocks4CommandResponse(Socks4CommandStatus.REJECTED_OR_FAILED, address.getHostAddress(),msg.dstPort());
                            ctx.writeAndFlush(commandResponse);
                        }
                    });
        }else {
